...
type testContainerGormContainer struct {
	container      testcontainers.Container
	defaultOptions *contracts.PostgresContainerOptions
}

func NewTestContainerGormContainer() *testContainerGormContainer {
	return &testContainerGormContainer{
		defaultOptions: &contracts.PostgresContainerOptions{
			Database:  "test_db",
			Port:      5432,
			Host:      "localhost",
...
func (g *testContainerGormContainer) Start(ctx context.Context, t *testing.T, options ...*contracts.PostgresContainerOptions) (*gorm.DB, error) {
	...
	port, err := nat.NewPort("tcp", strconv.Itoa(g.defaultOptions.Port))

	// get a free random host hostPort
	hostPort, err := dbContainer.MappedPort(ctx, port)
	if err != nil {
		return nil, err
	}
...
func (g *testContainerGormContainer) Cleanup(ctx context.Context) error {
	return g.container.Terminate(ctx)
}

func (g *testContainerGormContainer) getRunOptions(opts ...*contracts.PostgresContainerOptions) testcontainers.ContainerRequest {
	...
	port, _ := nat.NewPort("tcp", strconv.Itoa(g.defaultOptions.Port))
