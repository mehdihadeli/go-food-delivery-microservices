...
func (m *goosePostgresMigrator) executeCommand(command migration.CommandType, version uint) error {
	switch command {
	case migration.Up:
		if version == 0 {
		...
		return goose.Run("up-to VERSION ", m.db, m.config.MigrationsDir, strconv.FormatUint(uint64(version), 10))
	case migration.Down:
		if version == 0 {
			return goose.Run("down", m.db, m.config.MigrationsDir)
		}

		return goose.Run("down-to VERSION ", m.db, m.config.MigrationsDir, strconv.FormatUint(uint64(version), 10))
	default:
