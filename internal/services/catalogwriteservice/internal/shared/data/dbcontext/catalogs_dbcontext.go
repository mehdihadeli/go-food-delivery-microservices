...
func (c *CatalogsGormDBContext) ExistsProduct(
	ctx context.Context,
	id uuid.UUID,
) bool {
	var count int64

	c.WithContext(ctx).Model(&datamodel.ProductDataModel{}).Scopes(scopes.FilterByID(id)).Count(&count)
...
func (c *CatalogsGormDBContext) FindProductByID(
	...
	// result := c.WithContext(ctx).First(&productData, "id = ?", id)
	// result := c.WithContext(ctx).First(&datamodel.ProductDataModel{Id: id})
	// result := c.WithContext(ctx).Scopes(scopes.FilterByID(id)).First(&productData)
...
func (c *CatalogsGormDBContext) DeleteProductByID(
	...
	exists := c.ExistsProduct(ctx, id)
	if !exists {
		return customErrors.NewNotFoundError(fmt.Sprintf("product with id `%s` not found in the database",
			id.String(),
		))
	}
	...
	// result := dbContext.WithContext(ctx).Delete(&datamodel.ProductDataModel{Id: id})
	result := dbContext.WithContext(ctx).Delete(&datamodel.ProductDataModel{}, id)
	if result.Error != nil {
