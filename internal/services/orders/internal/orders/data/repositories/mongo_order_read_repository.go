...
func NewMongoOrderReadRepository(
	...
	return &mongoOrderReadRepository{log: log, cfg: cfg, mongoClient: mongoClient, tracer: tracer}
}
...
func (m mongoOrderReadRepository) GetAllOrders(
	...
	collection := m.mongoClient.Database(m.cfg.MongoDocumentOptions.DatabaseName).
		Collection(m.cfg.MongoDocumentOptions.Orders)
