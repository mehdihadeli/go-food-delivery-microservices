...
func TestCreateProduct(t *testing.T) {
	...
	Convey("Creating Product Feature", t, func() {
		...
		// scenario
		Convey("Creating a new product and saving it to the database for a none-existing product", t, func() {
			Convey("Given new product doesn't exists in the system", func() {
				...
				Convey("When the CreateProduct command is executed and product doesn't exists", func() {
					...
					Convey("Then the product should be created successfully", func() {
						...
						Convey("And the product ID should not be empty and same as commandId", func() {
							So(result.Id, ShouldEqual, command.Id)

							Convey("And product detail should be retrievable from the database", func() {
