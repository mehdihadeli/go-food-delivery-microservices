...
func (c *updateProductIntegrationTests) Test_Should_Consume_Product_Created_With_Existing_Consumer_From_Broker() {
	...
	newConsumer := messaging.ShouldConsume[*integration_events.ProductUpdatedV1](ctx, c.Bus, nil)
...
func (c *updateProductIntegrationTests) SetupSuite() {
	...
	err := c.Bus.ConnectConsumerHandler(&integration_events.ProductUpdatedV1{}, testConsumer2)
	c.Require().NoError(err)
