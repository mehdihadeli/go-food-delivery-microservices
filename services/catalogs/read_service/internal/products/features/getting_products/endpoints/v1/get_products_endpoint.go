...
func (ep *getProductsEndpoint) handler() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err != nil {
			...
			ep.GetLog().Errorf(fmt.Sprintf("[getProductsEndpoint_handler.GetListQueryFromCtx] err: %v", badRequestErr))
			return err
		}
		...
		if err := c.Bind(request); err != nil {
			...
			ep.GetLog().Errorf(fmt.Sprintf("[getProductsEndpoint_handler.Bind] err: %v", badRequestErr))
		...
		if err != nil {
			...
			ep.GetLog().Error(fmt.Sprintf("[getProductsEndpoint_handler.Send] err: {%v}", err))
			return err
		}
