...
func (c *createProductIntegrationTests) SetupTest() {
	...
	err := mediatr.RegisterRequestHandler[*CreateProduct, *dtos.CreateProductResponseDto](NewCreateProductHandler(c.Log, c.Cfg, c.ProductRepository, c.Bus))
	c.Require().NoError(err)

	testConsumer := consumer.NewRabbitMQFakeTestConsumerHandler[*v1.ProductCreatedV1]()
