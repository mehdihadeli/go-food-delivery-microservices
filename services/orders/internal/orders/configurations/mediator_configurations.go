...
func (c *ordersModuleConfigurator) configOrdersMediator(aggregateStore store.AggregateStore[*aggregate.Order]) error {
	...
	err = mediatr.RegisterRequestHandler[*gettingOrderByIdV1.GetOrderByIdQuery, *gettingOrderByIdDtos.GetOrderByIdResponseDto](gettingOrderByIdV1.NewGetOrderByIdHandler(c.Log, c.Cfg, aggregateStore))
	if err != nil {
		return err
	}
